...
print('+ Reading the events features...')
df_events = pd.read_csv("data/events.csv.zip", usecols=['uuid', 'document_id', 'platform'])
...
print('+ Reading the ads features...')
...
print('+ Reading the document meta features...')
df_doc_meta = pd.read_csv('data/documents_meta.csv.zip')

df_doc_meta.source_id.fillna(0, inplace=True)
df_doc_meta.source_id = df_doc_meta.source_id.astype('int32')
df_doc_meta.publisher_id.fillna(0, inplace=True)
...
print('+ Generating the data for test...')
