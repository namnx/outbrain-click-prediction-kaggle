...
print('+ defining subfolds for each fold...')
np.random.seed(1)
...
np.save('tmp/fold_0_split.npy', df_train_0.subfold.values)
np.save('tmp/fold_1_split.npy', df_train_1.subfold.values)


# split fold 0 into subfolds
print('+ split fold 0 into subfolds...')
f_0 = open('ffm/ffm_xgb_0_0.txt', 'w')
f_1 = open('ffm/ffm_xgb_0_1.txt', 'w')

with open('ffm/ffm_xgb_0.txt', 'r') as f_in:
    for fold, line in tqdm(zip(df_train_0.subfold, f_in)):
        if fold == 0:
            f_0.write(line)
        else:
            f_1.write(line)
...
with open('ffm/ffm_xgb_1.txt', 'r') as f_in:
    for fold, line in tqdm(zip(df_train_1.subfold, f_in)):
        if fold == 0:
            f_0.write(line)
        else:
            f_1.write(line)
